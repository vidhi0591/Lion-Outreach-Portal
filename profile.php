...
<?php if (isset($_GET['user_id'])) {
		...
		//check for existing following relation
		
		$query2 = "SELECT follow_relations FROM follow_connections WHERE user_id = ".$_SESSION['user']." && user_id_followed = ".$user_id."";  
		 
		$result2 = $db->query($query2);
		
		$row2 = $result2->fetch_assoc();
		
		if($row2['follow_relations']){
		
		echo "
		 
		 You are currently following ".$name."'s blog.
		 <br />
		 <form action='unfollowprocessing.php?follow_id=".$user_id."' method='post'>
    
		<input type='submit' name='follow' value='Click here to unfollow ".$name."'s blog!' />
		</form>	 
			 ";
		...
		echo "
		 ...
		 You are not following ".$name."'s blog.
		 <br />
		 <form action='followprocessing.php?follow_id=".$user_id."' method='post'>
    
		<input type='submit' name='follow' value='Click here to follow ".$name."'s blog!' />
		</form>	 
			 ";
